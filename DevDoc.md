...
## 2ea298c, ce14b59
- /mobile - GET을 통해 jwt 토큰으로 인증한 유저 관련 정보를 JSON으로 전송
- /mobile/login - GET을 통해 웹 브라우저에서도 테스트 가능한 환경 구성

##
- 채팅 이벤트 발생 시 자동 스크롤 기능 추가
- socket.js에서 `req.session`을 제대로 받아오지 못 하는 현상으로 인해 관련 라인 주석 처리
----------------------------------------
